...
def _repeat_dataproto_per_row(dp: DataProto, repeats: list[int]) -> DataProto:
    """Per-row non-uniform interleave repeat of a DataProto.

    repeats[i] is the number of times row i is duplicated in-place. Equivalent
    to torch.repeat_interleave / np.repeat with a 1-D `repeats` argument so that
    AVG@k benchmarks with different k can share one validation pass.
    """
    repeats_t = torch.as_tensor(repeats, dtype=torch.long)
    new_size = int(repeats_t.sum().item())
    if dp.batch is not None:
        repeated = {k: torch.repeat_interleave(v, repeats_t, dim=0) for k, v in dp.batch.items()}
        new_batch = TensorDict(source=repeated, batch_size=(new_size,))
    else:
        new_batch = None
    repeats_np = np.asarray(repeats, dtype=np.int64)
    new_nt = {k: np.repeat(v, repeats_np, axis=0) for k, v in dp.non_tensor_batch.items()}
    return type(dp)(batch=new_batch, non_tensor_batch=new_nt, meta_info=dp.meta_info)
...
class AgentRayPPOTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # repeat test batch (per-row, controlled by extra_info["n_samples"])
            default_n = self.config.actor_rollout_ref.rollout.val_kwargs.n
            extra_info_arr = test_batch.non_tensor_batch.get("extra_info", None)
            if extra_info_arr is not None:
                repeats = []
                for ei in extra_info_arr:
                    n = None
                    if isinstance(ei, dict):
                        n = ei.get("n_samples", None)
                    repeats.append(int(n) if n is not None else int(default_n))
            else:
                repeats = [int(default_n)] * len(test_batch.batch)
            test_batch = _repeat_dataproto_per_row(test_batch, repeats)
