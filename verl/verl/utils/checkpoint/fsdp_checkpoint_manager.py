...
def has_none_key(obj):
    """
    递归检查字典中是否存在键为 None 的情况。
    :param obj: 待检查的对象（通常是 model.config.to_dict()）
    :return: 存在 None 键返回 True，否则返回 False
    """
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.should_save_hf_model:
            ...
            if self.rank == 0:
                ...
                if has_none_key(save_model.config.to_dict()):
                    print("⚠️ 警告：检测到配置中存在 None 键！兜底策略")
                    save_model.config.auto_map = {
