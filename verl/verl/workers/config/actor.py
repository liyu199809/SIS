...
class FSDPActorConfig(ActorConfig):
    ...
    profiler: ProfilerConfig = field(default_factory=ProfilerConfig)
    use_rollout_log_probs: bool = False
    use_clip_less: bool = False
    use_sis_kl: bool = False
    use_policy_clip: bool = True
    log_token_acceptance: bool = False
    token_acceptance_dir: Optional[str] = None
    token_acceptance_log_first_epoch_only: bool = True
    # Number of top-K logits kept for SIS (clip-less) topk / gather and SIS-KL.
    topk_k: int = 100
    # Coarse SIS forward-compute profiling (no cuda.synchronize). Off by default.
    sis_compute_profiling: bool = False
