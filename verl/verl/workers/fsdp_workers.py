...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(
                ...
                timeout=datetime.timedelta(seconds=self.config.get("nccl_timeout", 3000)),
        ...
        self.use_clip_less = self.config.actor.get("use_clip_less", False)
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, out = self.actor.compute_log_prob(data=data, calculate_entropy=True, return_topk=self.use_clip_less)
            output = DataProto.from_dict(
                tensors={"old_log_probs": output, "entropys": entropys, **out},
