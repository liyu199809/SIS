...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_clip_less = self.config.actor_rollout_ref.actor.get("use_clip_less", False)
        self.use_sis_kl = self.config.actor_rollout_ref.actor.get("use_sis_kl", False)

        # define in-reward KL control
        # kl loss control currently not suppoorted
        if self.config.algorithm.use_kl_in_reward:
            self.kl_ctrl_in_reward = core_algos.get_kl_controller(self.config.algorithm.kl_ctrl)
            self.use_sis_kl = False; self.config.actor_rollout_ref.actor["use_sis_kl"] = False
            print("#########\nuse_sis_kl=False, because use_kl_in_reward=True\n#########")
