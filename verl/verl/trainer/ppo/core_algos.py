...
# Module-level accumulator for SIS loss-compute profiling (offpolicy2onpolicy).
# Populated only inside offpolicy2onpolicy() via marked_timer; flushed by
# DataParallelPPOActor._flush_sis_timing() at the end of update_policy.
# NOTE: marked_timer does NOT cuda.synchronize() — async-launch approximation.
SIS_LOSS_TIMING: dict[str, float] = {}
# Toggle set by DataParallelPPOActor.__init__ from config.sis_compute_profiling.
# When False, offpolicy2onpolicy skips timing entirely (no overhead, no accumulation).
SIS_PROFILING_ENABLED: bool = False


def _sis_loss_timer(name: str):
    """Context manager for SIS loss-compute profiling. No-op when disabled."""
    if SIS_PROFILING_ENABLED:
        return marked_timer(name, SIS_LOSS_TIMING)
...
def compute_policy_loss(
    ...
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, 0.0, 0.0, 0.0, None


def offpolicy2onpolicy(
        negative_approx_kl, log_prob_topk,
        old_log_prob_topk, response_mask,
        cliprange_low=0.2, cliprange_high=0.2
):
    # SIS-only compute (baseline does not call this). Coarse timing for profiling.
    with _sis_loss_timer("sis/offpolicy2onpolicy"):
        negative_approx_kl_topk = log_prob_topk - old_log_prob_topk
        negative_approx_kl_topk = torch.clamp(negative_approx_kl_topk, min=-20.0, max=20.0)
        log_M = negative_approx_kl_topk.max(dim=-1).values
        y = torch.rand_like(negative_approx_kl)
        sample_condition = torch.log(y) < (negative_approx_kl - log_M)
        zero_condition = negative_approx_kl == 0
        condition = sample_condition | zero_condition
        accept_mask = condition.float() * response_mask
        accept_rate = (accept_mask.sum() / response_mask.sum()).item()

        # 计算 D 值 (per-sequence mean)
        seq_lengths = response_mask.sum(dim=-1).clamp(min=1)
        abs_log_ratio_raw = negative_approx_kl.abs() * response_mask
        log_clip_low = math.log(1 - cliprange_low) if cliprange_low < 1 else None  # >=1: ratio 下界非正(ratio 恒正)，传 None 不裁剪下界
        log_clip_high = math.log(1 + cliprange_high)
        clipped_neg_kl = torch.clamp(negative_approx_kl, min=log_clip_low, max=log_clip_high)
        abs_log_ratio_clipped = clipped_neg_kl.abs() * response_mask
        # D_original: 接受的 token 保持原始值; 被拒绝的 token 考虑 clip (被拒绝+满足clip条件才被clip)
        D_original = ((abs_log_ratio_raw * accept_mask + abs_log_ratio_clipped * (1 - accept_mask)).sum(dim=-1) / seq_lengths).mean().item()
        # D_SIS: 接受的 token = 0 (on-policy); 被拒绝的 token 考虑 clip
        D_sis = ((abs_log_ratio_clipped * (1 - accept_mask)).sum(dim=-1) / seq_lengths).mean().item()

        negative_approx_kl = negative_approx_kl - (accept_mask * negative_approx_kl).detach()
        return negative_approx_kl, accept_rate, accept_mask, D_original, D_sis
...
def compute_policy_loss_vanilla(
    ...
    old_log_prob_topk: torch.Tensor | None = None,
    log_prob_topk: torch.Tensor | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Compute the clipped policy objective and related metrics for PPO.
    ...
    use_clip_less = config.get("use_clip_less", False)
    use_policy_clip = config.get("use_policy_clip", True)
    ...
    if use_clip_less:
        negative_approx_kl, accept_rate, accept_mask_out, D_original, D_sis = offpolicy2onpolicy(
            negative_approx_kl, log_prob_topk, old_log_prob_topk, response_mask,
            cliprange_low=cliprange_low, cliprange_high=cliprange_high
        )
    else:
        accept_rate = 0.0
        accept_mask_out = None
        # 即使不用 SIS，也统计 D_original 用于对比
        abs_log_ratio = negative_approx_kl.abs() * response_mask
        seq_lengths = response_mask.sum(dim=-1).clamp(min=1)
        D_original = (abs_log_ratio.sum(dim=-1) / seq_lengths).mean().item()
        # baseline 没有 SIS，所有 token 都被 clip
        log_clip_low = math.log(1 - cliprange_low) if cliprange_low < 1 else None  # >=1: ratio 下界非正(ratio 恒正)，传 None 不裁剪下界
        log_clip_high = math.log(1 + cliprange_high)
        clipped_neg_kl = torch.clamp(negative_approx_kl, min=log_clip_low, max=log_clip_high)
        abs_log_ratio_clipped = clipped_neg_kl.abs() * response_mask
        D_sis = (abs_log_ratio_clipped.sum(dim=-1) / seq_lengths).mean().item()

    ratio = torch.exp(negative_approx_kl) # original PPO
    # NOTE: Set importance sampling weights to 1
    # ratio = torch.ones_like(negative_approx_kl) 
    pg_losses1 = -advantages * ratio
    if use_policy_clip:
        if cliprange_low is None:
            cliprange_low = cliprange
        ...
        # No clipping: use original pg_losses1 directly
    ...
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, accept_rate, D_original, D_sis, accept_mask_out
...
def compute_policy_loss_gspo(
    ...
    use_clip_less = config.get("use_clip_less", False)
    if use_clip_less and (old_log_prob_topk is None or log_prob_topk is None):
        raise ValueError("old_log_prob_topk and log_prob_topk must be provided when use_clip_less is True.")
    ...
    negative_approx_kl = log_prob - old_log_prob
    if use_clip_less:
        negative_approx_kl, accept_rate, accept_mask, D_original, D_sis = offpolicy2onpolicy(
            negative_approx_kl, log_prob_topk, old_log_prob_topk, response_mask
        )
    else:
        accept_rate = 0.0
        accept_mask = torch.zeros_like(response_mask)
        # 即使不用 SIS，也统计 D_original 用于对比
        abs_log_ratio = negative_approx_kl.abs() * response_mask
        seq_lengths = response_mask.sum(dim=-1).clamp(min=1)
        D_original = (abs_log_ratio.sum(dim=-1) / seq_lengths).mean().item()
    ...
    offpolicy_mask = response_mask * (1 - accept_mask)
    onpolicy_mask = response_mask * accept_mask

    offpolicy_seq_lengths = torch.sum(offpolicy_mask, dim=-1).clamp(min=1)
    onpolicy_seq_lengths = torch.sum(onpolicy_mask, dim=-1).clamp(min=1)

    offpolicy_kl_seq = torch.sum(negative_approx_kl * offpolicy_mask, dim=-1) / offpolicy_seq_lengths
    onpolicy_kl_seq = torch.sum(negative_approx_kl * onpolicy_mask, dim=-1) / onpolicy_seq_lengths

    log_offpolicy_seq_ratio = log_prob - log_prob.detach() + offpolicy_kl_seq.detach().unsqueeze(-1)
    log_offpolicy_seq_ratio = torch.clamp(log_offpolicy_seq_ratio, max=10.0) # 防止数值溢出
    offpolicy_seq_ratio = torch.exp(log_offpolicy_seq_ratio)

    gspo_losses1 = -advantages * offpolicy_seq_ratio
    gspo_losses2 = -advantages * torch.clamp(offpolicy_seq_ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
    gspo_losses = torch.maximum(gspo_losses1, gspo_losses2)

    log_onpolicy_seq_ratio = log_prob - log_prob.detach() + onpolicy_kl_seq.detach().unsqueeze(-1)
    log_onpolicy_seq_ratio = torch.clamp(log_onpolicy_seq_ratio, max=10.0)
    onpolicy_seq_ratio = torch.exp(log_onpolicy_seq_ratio)

    normal_losses = -advantages * onpolicy_seq_ratio
    pg_losses = onpolicy_mask * normal_losses + offpolicy_mask * gspo_losses
    ...
    # gspo_losses1 = -advantages * seq_importance_ratio
    # gspo_losses2 = -advantages * torch.clamp(seq_importance_ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
    # gspo_losses = torch.maximum(gspo_losses1, gspo_losses2)

    # # On-Policy Data Trianing
    # normal_ratio = torch.exp(log_prob - log_prob.detach())
    # normal_losses = -advantages * normal_ratio

    # pg_losses = accept_mask * normal_losses + (1 - accept_mask) * gspo_losses # merge it
    ...
    pg_clipfrac = verl_F.masked_mean(torch.gt(gspo_losses2, gspo_losses1).float(), response_mask)
    pg_clipfrac_lower = torch.tensor(0.0, device=pg_loss.device)

    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, accept_rate, D_original, D_sis, accept_mask
...
def compute_policy_loss_cispo(
    ...
    old_log_prob_topk: torch.Tensor | None = None,
    log_prob_topk: torch.Tensor | None = None,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, float, float, float, torch.Tensor | None]:
    """Compute the CISPO policy objective (clipped IS-weighted REINFORCE).

    See https://arxiv.org/pdf/2506.13585.

    CISPO loss:
        L = - sg(clip(ratio, 1-eps_low, 1+eps_high)) * A * log_pi_theta
    where the clipped ratio is detached so gradients flow only through log_prob.

    When `config.use_clip_less` is True we additionally apply the off-policy ->
    on-policy SIS correction (`offpolicy2onpolicy`) on `negative_approx_kl`
    before exponentiating, so that "accepted" tokens are treated as on-policy
    (their importance weight collapses to 1) while "rejected" tokens carry
    the standard IS ratio.
    """
    ...
    # Compute log importance ratio: log(pi_theta / pi_theta_old)
    negative_approx_kl = log_prob - old_log_prob
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)

    # Optional off-policy -> on-policy SIS correction (same machinery as vanilla/gspo).
    if use_clip_less:
        negative_approx_kl, accept_rate, accept_mask_out, D_original, D_sis = offpolicy2onpolicy(
            negative_approx_kl, log_prob_topk, old_log_prob_topk, response_mask,
        ...
        seq_lengths = response_mask.sum(dim=-1).clamp(min=1)
        abs_log_ratio_raw = negative_approx_kl.abs() * response_mask
        D_original = (abs_log_ratio_raw.sum(dim=-1) / seq_lengths).mean().item()
        log_clip_low = math.log(1 - clip_ratio_low) if clip_ratio_low < 1 else None  # >=1: ratio 下界非正(ratio 恒正)，传 None 不裁剪下界
        log_clip_high = math.log(1 + clip_ratio_high)
        clipped_neg_kl = torch.clamp(negative_approx_kl, min=log_clip_low, max=log_clip_high)
        abs_log_ratio_clipped = clipped_neg_kl.abs() * response_mask
        D_sis = (abs_log_ratio_clipped.sum(dim=-1) / seq_lengths).mean().item()
    ...
    # CISPO core: clipped, *detached* IS weights times log_prob (REINFORCE-style).
    # Gradient flows ONLY through log_prob.
    ...
    # Track clipping statistics on the (un-detached) ratio.
    ...
    # CISPO has no lower-clip branch (no dual-clip).
    pg_clipfrac_lower = torch.tensor(0.0, device=pg_loss.device)
...
def compute_policy_loss_gpg(
    ...
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
    return pg_loss, torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0), 0.0, 0.0, 0.0, None
...
def compute_policy_loss_clip_cov(
    ...
    return pg_loss, pg_clipfrac, ppo_kl, torch.tensor(0.0), 0.0, 0.0, 0.0, None
...
def compute_policy_loss_kl_cov(
    ...
    return pg_loss, torch.tensor(0.0), ppo_kl_abs, torch.tensor(0.0), 0.0, 0.0, 0.0, None
...
def calculate_sis_kl(log_prob_topk, old_log_prob_topk, response_mask):
    """
    计算截断在 Top-K 上的精确 KL 散度，并在 K 维度上重新归一化。
    
    参数:
    log_prob_topk: [batch_size, seq_len, k] - 当前策略的 top-k log 概率
    old_log_prob_topk: [batch_size, seq_len, k] - 旧策略的 top-k log 概率
    response_mask: [batch_size, seq_len] - 用于过滤 padding 和 prompt 的掩码
    """
    
    # 1. 在 K 维度上对旧策略和新策略进行归一化
    old_log_prob_topk_norm = F.log_softmax(old_log_prob_topk, dim=-1)
    log_prob_topk_norm = F.log_softmax(log_prob_topk, dim=-1)
    
    kl_topk = F.kl_div(
        input=log_prob_topk_norm,
        target=old_log_prob_topk_norm,
        reduce='none',
        log_target=True
    )
    
    # [batch_size, seq_len]
    approx_kl_token = kl_topk.sum(dim=-1)
    
    valid_kl = approx_kl_token * response_mask
    kl_loss = valid_kl.sum() / response_mask.sum()
