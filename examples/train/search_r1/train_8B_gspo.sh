...
prefix_name="fix_bug"
if [ "$is_clip_less" = "True" ]; then
    run_name_postfix="${prefix_name}-${loss_mode}-clip-less"
else
    run_name_postfix="${prefix_name}-${loss_mode}"
fi
...
python3 -m verl_tool.servers.serve --host $host --port $port --tool_type "search_retrieval" --workers_per_tool 8 --max_concurrent_requests=512 &
server_pid=$!
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.use_clip_less=$is_clip_less \
