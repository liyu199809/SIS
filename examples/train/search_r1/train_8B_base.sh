...
set -x
model_name="$PREFIX/base_model/Llama-3.1-8B-Instruct"
...
use_clip_less=False # use sis for reward calculation
use_policy_clip=True # whether to use policy gradient clipping
n_gpus_per_node=8
n_nodes=1
n=8
total_epochs=15
...
model_pretty_name=$(basename $model_name | tr '/' '_' | tr '[:upper:]' '[:lower:]')
run_name_postfix="baseline"
if [ "$use_policy_clip" = "False" ]; then
    run_name_postfix+="no_clip"
fi
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.use_policy_clip=$use_policy_clip \
